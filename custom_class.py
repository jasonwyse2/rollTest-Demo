...
import pandas as pd
import os
from data.load_data import get_datasets_2, get_x_y_sharpGentleUpDown, get_x_y_bottomTopUpDown
...
class Your_Data(base.Data):
    ...
    def _run(self):
        train_valid_type = self._data_parameters_obj._args['train_valid_type']
    ...
    def __split_train_valid(self, raw_sample_mat):
        ...
        for i in range(closeSeries_num):
            ...
            train_x, train_y = x[:-(valid_lookBack_from_endTime)], y[:-(valid_lookBack_from_endTime)]
            valid_x, valid_y = x[-(valid_lookBack_from_endTime):], y[-(valid_lookBack_from_endTime):]
    ...
    def _prepare_data(self,dataType=''):
        ...
        show_label = self._data_parameters_obj._args['show_label']
        fourlabelType = self._data_parameters_obj._args['fourlabelType']
        ...
        for i in range(closeSeries_num):
            ...
            if fourlabelType == 'SharpGentleUpDown':
                x, y, filtered_close_for_use, close_for_use = get_x_y_sharpGentleUpDown(close, self._data_parameters_obj._args)
            elif fourlabelType == 'BottomTopUpDown':
                x, y, filtered_close_for_use, close_for_use = get_x_y_bottomTopUpDown(close, self._data_parameters_obj._args)
            if show_label == True:
    ...
    def _prepare_test_data(self):
        ...
        extra_end = self._data_parameters_obj._args['extraTradeDays_afterEndTime']
        fourlabelType = self._data_parameters_obj._args['fourlabelType']
        ...
        if fourlabelType == 'SharpGentleUpDown':
            x, y, filtered_close_for_use, close_for_use = get_x_y_sharpGentleUpDown(close,
                                                                                    self._data_parameters_obj._args)
        elif fourlabelType == 'BottomTopUpDown':
